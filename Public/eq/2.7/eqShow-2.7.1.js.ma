...
function(a, b, c) {
	...
	f.prototype = {
		...
		d.addInterceptor(function(a, b) {
			...
			for (var b = $(a).find("a[data]"), c = 0; c < b.length; c++) if (b[c] && "view" == d.mode) {
	...
	b.module("app", ["ngRoute", "home", "sample", "main", "reg", "scene", "my", "data", "data.edit", "error", "usercenter", "ui.bootstrap", "ui.grid", "ui.grid.selection", "ngSanitize", "ui.select", "services.i18nNotifications", "services.httpRequestTracker", "services.sample", "services.config", "security", "app.upload", "templates-app", "templates-common", "ui.sortable", "I18N.MESSAGES", "app.directives.notification"]), b.module("app").config(["$routeProvider", "$locationProvider", "securityAuthorizationProvider", "uiSelectConfig", function(a, b, c, d) {
		...
		b.getDataDetail = function() {
			j.getDataDetail(b.sceneId).then(function(a) {
				b.dataDetail = a.data.obj, l = a.data.obj, b.groupNames = b.dataDetail.group;
			...
			var c = {};
			c.id = b.sceneId, c.sex = 0 !== a.id ? a.name : "", l.sex = c.sex
		...
		b.assignGroup = function() {
			...
			if (!m.length) return void alert("您还没有选择分组!");
			var f = {
	...
	}]), b.module("error", ["services.sample"]), b.module("error").controller("ErrorCtrl", ["$rootScope", "$http", "$scope", "$timeout", "security", "$window", "sampleService", function() {}]), b.module("home", ["services.sample", "app.directives.addelement", "services.scene", "app.directives.qrcode", "app.directives.loading"]), b.module("home").controller("HomeCtrl", ["$rootScope", "$http", "$scope", "$timeout", "security", "$window", "sampleService", "sceneService", "$routeParams", "$route", "$location", function(a, b, c, d, e, f, g, h, i, j, k) {
	...
	}]), b.module("customer.group", ["services.data"]), b.module("customer.group").controller("AddGroupCtrl", ["$rootScope", "$scope", "$window", "$routeParams", "dataService", "security", "$modal", "ModalService", "$location", function(a, b, c, d, e) {
	...
	}]), b.module("main.data", ["app.directives.dataDraggable", "customer.group", "services.i18nNotifications", "app.directives.customer"]), b.module("main.data").controller("CustomerCtrl", ["$scope", "$route", "$location", "dataService", "$modal", "ModalService", "i18nNotifications", "security", function(a, b, c, d, e, f, g, h) {
		function i(b) {
		...
		a.PREFIX_URL = PREFIX_URL, a.isActive = "customer", a.select = 0, a.staticFileds = [{
			...
			f.openConfirmDialog({
				...
				d.deleteDataById(b.id).then(function() {
					i(1 === a.customerDatas.length && a.currentPage > 1 ? a.currentPage - 1 : a.currentPage), k()
				})
			})
			...
			return 1 > b || b > a.totalItems / 10 + 1 ? void alert("此页超出范围") : void i(b)
		}, a.editCustom = function(a) {
		...
		if (a.confirm = function() {
			l ? jQuery.isEmptyObject(a.associateData, {}) ? (alert("请导入数据！"), l = !0) : (d.mergeSceneData(a.selectedSceneId, a.associateData).then(function() {
				alert("你已成功导入客户！"), b.reload()
			}, function() {}), l = !1) : alert("请不要重复提交！")
		}, a.importDatas(), j(), k(), i(0), a.isAllowedToAccessGrouping = h.isAllowToAccess(h.accessDef.GROUP_CUSTOMER), a.isAllowedToAccessGrouping) {
			a.allImages = {
				selected: !1
				...
				e.open({
					...
					a.groups.unshift(b), p(), g.pushForCurrentRoute("group.create.success", "notify.success")
				}, function() {})
			};
			var m = [],
				n = [];
			a.assignGroup = function() {
				m = [], n = [];
				for (var b = 0, c = a.customerDatas.length; c > b; b++) a.customerDatas[b].selected && m.push(a.customerDatas[b].id);
				for (b = 0, c = a.groups.length; c > b; b++) a.groups[b].selected && n.push(a.groups[b].id);
				if (!m.length) return void alert("您还没有选择客户!");
				...
				d.assignGroup(e).then(function(b) {
					b.data.success && (p(), a.allImages.selected = !1, o(), g.pushForCurrentRoute("data.assign.success", "notify.success"))
				}, function() {})
				...
				if (b) c = {
					...
					for (var h = 0, i = a.customerDatas.length; i > h; h++) a.customerDatas[h].selected && m.push(a.customerDatas[h].id);
					if (!m.length) return void alert("您还没有选择客户！");
					c = {
						ids: m
					}, e = "确认删除选中数据？"
				...
				f.openConfirmDialog({
					...
					d.deleteCustomer(c).then(function(b) {
						b.data.success && (a.allImages.selected = !1, o(), p(), g.pushForCurrentRoute("data.delete.success", "notify.success"))
					}, function() {
						alert("服务器异常！")
					})
				})
				...
				f.openConfirmDialog({
					...
					d.deleteGroup(b.id).then(function(b) {
						b.data.success && (a.groups.splice(c, 1), p(), g.pushForCurrentRoute("group.delete.success", "notify.success"))
					}, function() {
						alert("服务器异常!")
					})
				})
			...
			var o = function() {
					i(1 === a.customerDatas.length && a.currentPage > 1 ? a.currentPage - 1 : a.currentPage)
				},
				p = function() {
	...
	}]), b.module("main.spread", ["app.directives.pieChart", "app.directives.numChangeAnim"]), b.module("main.spread").controller("SpreadCtrl", ["$scope", "$location", "MineService", "dataService", function(a, b, c, d) {
	...
	}]), b.module("main.spread.detail", ["services.spread", "app.directives.lineChart", "app.directives.pieChart", "app.directives.numChangeAnim"]), b.module("main.spread.detail").controller("SpreadDetailCtrl", ["$scope", "$location", "$routeParams", "sceneService", "SpreadService", function(a, b, c, d, e) {
		...
		a.getAllStats = function() {
			...
			b.path("main/customer")
		}, g(), a.expandWebs = new Array([1]), a.addWeb = function(a) {
			a.push("")
		}, a.updataData = function() {}, a.deleteWeb = function(a, b) {
		...
		document.getElementById("sharescript") ? ($("#sharescript").remove(), r = document.getElementsByTagName("head")[0], s = document.createElement("script"), s.id = "sharescript", s.src = "http://bdimg.share.baidu.com/static/api/js/share.js?cdnversion=" + ~ (-new Date / 36e5), r.appendChild(s)) : (r = document.getElementsByTagName("head")[0], s = document.createElement("script"), s.id = "sharescript", s.src = "http://bdimg.share.baidu.com/static/api/js/share.js?cdnversion=" + ~ (-new Date / 36e5), r.appendChild(s)), g.getSceneDetail = function() {
			i.getSceneDetail(g.sceneId).then(function(b) {
				g.scene = b.data.obj, g.scene.applyPromotion = "" + g.scene.applyPromotion, g.scene.applyTemplate = "" + g.scene.applyTemplate, g.code = PREFIX_URL + "eqs/qrcode/" + g.scene.code + ".png", g.url = PREFIX_HOST + "v-" + g.scene.code, g.customUrl = l.trustAsResourceUrl(PREFIX_HOST + "v-"+ g.scene.id + "&preview=preview"), g.scene.image.isAdvancedUser = e.isAdvancedUser || e.isVendorUser ? !0 : !1, a._bd_share_config = {
					common: {
						bdText: g.scene.name,
						bdDesc: g.scene.name,
						bdUrl: g.url,
	...
	}]), b.module("scene.create.console.bg", ["services.file", "scene.my.upload", "app.directives.responsiveImage", "app.directives.rightclick"]), b.module("scene.create.console.bg").controller("BgConsoleCtrl", ["$scope", "$timeout", "$rootScope", "$modal", "ModalService", "sceneService", "fileService", "localizedMessages", "obj", function(a, b, c, d, e, f, g, h, i) {
		...
		a.createCategory = function() {
			...
			if (c.target.id != b.id) if (b.selected = !b.selected, b.selected) a.selectedImages.push(b);
			else for (var d in a.selectedImages) a.selectedImages[d] == b && a.selectedImages.splice(d, 1)
			...
			if (!b) for (var h in a.imgList) a.imgList[h].selected && d.push(a.imgList[h].id);
			var i = b ? b : d.join(",");
			e.openConfirmDialog({
				msg: f
			}, function() {
		...
		return {
			...
			link: function(c, d) {
				...
				var g, h, i, j, k, l, m, n = a.currentElemDef,
					o = a.currentElemDef.properties.src,
					p = $("#target"),
					q = n.css.width / n.css.height;
				c.fit = !0, c.lockRatio = !1, c.$on("changeElemDef", function(a, b) {
					...
					g ? (e(), g.setOptions({
						aspectRatio: q,
						setSelect: h
					})) : p.attr("src", PREFIX_FILE_HOST + a).load(function() {
						l = this.width, m = this.height, p.Jcrop({
							onChange: f,
							keySupport: !1,
					...
					if (g) {
						var b = g.tellSelect();
						b.w = parseInt(b.w, 10), b.h = parseInt(b.h, 10), g.setOptions(a ? {
							aspectRatio: b.w / b.h
						} : {
							aspectRatio: null
						})
					...
					if (g) if (a) {
						var b = g.tellSelect();
						b.x = parseInt(b.x, 10), b.y = parseInt(b.y, 10), b.x2 = parseInt(b.x2, 10), b.y2 = parseInt(b.y2, 10), i = [b.x, b.y, b.x2, b.y2], g.setOptions({
							aspectRatio: q,
							setSelect: h
						})
					} else g.setOptions({
		...
		f.saveScene = function(a, c) {
			...
			if (e.user) {			 
				var b = e.mySceneId || JSON.parse(e.user.property);
				if (b) {
					var c = e.mySceneId || JSON.parse(e.user.property).myTplId;
					a.sceneId = c
				} else a.sceneId = null;
		...
		f.getPageTplsByMyType = function() {
			if (!D) if (e.mySceneId) D = e.mySceneId;
			else {
				var a = JSON.parse(e.user.property);
				a && (D = e.mySceneId = a.myTplId)
			}
			D ? k.previewScene(D).then(function(a) {
				f.myPageTpls = a.data.list
			}) : f.myPageTpls = []
		}, f.$on("myPageList.update", function() {
		...
		f.getPageTagLabel = function(a) {
			H[a] ? (f.pageLabel = H[a], J()) : l.getPageTagLabel(a).then(function(b) {
				f.pageLabel = H[a] = b.data.list, J()
			})
		}, f.pageLabelAll = [];
		var I, J = function() {
		...
		return {
			...
			replace: !1,
			template: '<div class="delete-element" ng-click="deleteMyTpl($event)" title="删除此模板"><img ng-src="{{CLIENT_CDN}}assets/images/bg_07.png" /></div>',
			link: function(c, d) {
				c.deleteMyTpl = function(d) {
					d.stopPropagation(), b.openConfirmDialog({
						msg: "确定删除此模板?"
					}, function() {
						a.deletePage(c.myTpl.id).then(function() {
							c.$emit("myPageList.update")
						}, function() {
							alert("服务器异常!")
						})
					})
		...
		return {
			...
			link: function(b, c) {
				...
				a.getBanners().then(function(a) {
					...
					d.append('<img src="' + CLIENT_CDN + 'assets/images/slide_03.png" width="1000px" height="720px" alt="Slide 1">'), d.append('<img src="' + CLIENT_CDN + 'assets/images/slide2_03.png" width="1000px" height="720px" alt="Slide 1">'),  $(c).slides({
						preload: !0,
						play: 5e3,
						pause: 2500,
						hoverPause: !0
		...
		function d(a, d, f, g) {
			...
			y.get("pan").set({
				...
				v.css("cursor", "default"), $("body").css({
					"user-select": "initial",
					cursor: "default"
				}), c(a, u), a.$broadcast("updateMaxRadius", u)
			})
		}
		var e = {
			RESIZE_W: "w-resize",
		...
		return {
			...
			link: function(a, b) {
				var c = b,
					f = $('<div class="bar bar-n"><div class="bar-radius"></div></div>'),
					g = $('<div class="bar bar-s"><div class="bar-radius"></div></div>'),
					h = $('<div class="bar bar-e"><div class="bar-radius"></div></div>'),
	...
	}]), b.module("security.register.form", ["services.localizedMessages", "app.directives.register"]), b.module("security.register.form").controller("RegisterFormController", ["$scope", "$timeout", "security", "localizedMessages", "$location", "$http", "$window", function(a, b, c, d, e) {
		...
		return f
	}]), b.module("security.service", ["security.retryQueue", "security.login", "security.register", "security.authority", "ui.bootstrap.modal"]).factory("security", ["$http", "$q", "$location", "securityRetryQueue", "$modal", "ModalService", "authority", function(b, c, d, e, f, g, h) {
		function i(b) {
			b = b || "/", a.location.href = b
		}
		function j() {
			if (t && (k(t, !1), t = null), r) throw new Error("Trying to open a dialog that is already open!");
			r = f.open({
				windowClass: "login-container",
				keyboard: !1,
		...
		function l(a) {
			r = null, a ? ("/home/login" == d.path() && d.path("/home", !1), e.retryAll()) : (e.cancelAll(), i())
		}
		function m(a) {
		...
		function n() {
			if (r && (k(r, !0), r = null), t) throw new Error("Trying to open a dialog that is already open!");
			t = f.open({
				windowClass: "login-container",
				keyboard: !1,
			...
			w = {
				...
				login: function(a) {
					...
					return e.then(function(a) {
						if (200 === a.status) {
							...
							("/home" == d.path() || "/home/login" == d.path()) && d.path("main"), w.requestCurrentUser(), k(r, !0)
				...
				register: function(a) {
					...
					return c.then(function(a) {
						if (200 === a.status) {
							...
							("/home" == d.path() || "/home/register" == d.path()) && d.path("main"), w.requestCurrentUser(), k(t, !0)
						} else w.isAuthenticated()
					}, function() {
						w.isAuthenticated()
					})
				...
				thirdPartLogin: function(a) {
					...
					return c.then(function(a) {
						if (200 === a.status) {
							if (a.data.success !== !0) return a.data;
							("/home" == d.path() || "/home/login" == d.path()) && d.path("main"), w.requestCurrentUser(), k(u, !0)
						} else w.isAuthenticated()
					}, function() {
						w.isAuthenticated()
					})
				...
				hasRel: function(a) {
					...
					f.then(function(b) {
						200 === b.status ? b.data.success === !0 ? (("/home" == d.path() || "/home/login" == d.path()) && d.path("main"), w.requestCurrentUser()) : "未关联账号" == b.data.msg && o(a) : w.isAuthenticated()
					}, function() {
						w.isAuthenticated()
					})
				},
				...
				thirdPartyUrl: {
					weiChatUrl: "https://open.weixin.qq.com/connect/qrconnect?appid=wxc5f1bbae4bb93ced&redirect_uri=http%3A%2F%2Flight.app52.cn&response_type=code&scope=snsapi_login&state=STATE#wechat_redirect",
					qqUrl: "https://graph.qq.com/oauth2.0/authorize?response_type=token&client_id=101149132&redirect_uri=http%3A%2F%2Flight.app52.cn&scope=get_user_info",
					weiboUrl: "https://api.weibo.com/oauth2/authorize?client_id=3508809852&response_type=token&redirect_uri=http://light.app52.cn"
				},
				isAllowToAccess: function(a) {
					for (var b = 0; b < w.currentUser.roleIdList.length; b++) {
						var c = h.userRoleDef[w.currentUser.roleIdList[b]];
						if (c && (c.code & a) > 0) return !0
					}
					return !1
				},
				...
				isAdvancedUser: function() {
					...
					var a = w.currentUser.roleIdList;
					if (!a) return !1;
					for (var b = 0; b < a.length; b++) if ("10" == a[b]) return !0;
					return !1
				},
				isVendorUser: function() {
					...
					var a = w.currentUser.roleIdList;
					if (!a) return !1;
					for (var b = 0; b < a.length; b++) if ("11" == a[b]) return !0;
					return !1
				},
		...
		return b.getDataBySceneId = function(b, c, d) {
			...
			var c = "?c=custom&a=getAllData&pageSize=10&pageNo=" + b,
				d = new Date;
			return c += "&time=" + d.getTime(), a({
				withCredentials: !0,
				method: "GET",
			...
			var b = "?c=custom&a=prospectCount&time=" + (new Date).getTime();
			return a({
				withCredentials: !0,
				method: "GET",
				url: JSON_URL + b
		...
		return b.listFileCategory = function(b) {
			...
			var c ="?c=page&a=crop&"; // "m/base/file/crop";
			return a({
				withCredentials: !0,
				method: "POST",
				url: JSON_URL + c,
	...
	}]), b.module("services.scene", ["scene.create.console", "services.history"]), b.module("services.scene").factory("sceneService", ["$http", "$rootScope", "$modal", "$q", "security", "$cacheFactory", "historyService", function(a, c, d, e, f, g, h) {
		function i(a) {
			H.obj.elements = a, $("#nr").empty(), G.parse({
				def: H.obj,
				appendTo: "#nr",
				mode: "edit"
			}), $("#editBG").hide();
		...
		function k(a, b) {
			var c = parseInt(a.css.top.substring(0, a.css.top.length - 2), 10) + 34 * F.sameCopyCount,
				d = parseInt(a.css.left.substring(0, a.css.left.length - 2), 10);
			c + 34 > $("#nr .edit_area").outerHeight() ? (b.css.top = c + "px", b.css.left = d + 10 + "px") : (b.css.top = c + 34 + "px", b.css.left = a.css.left, F.sameCopyCount++)
		}
		function l() {
			for (var a = Math.ceil(1e3 * Math.random()), b = 0; b < I.length; b++) if (I[b].id == a) return l();
			return a
		}
		function m(a, b, c) {
			var d, e = l(),
			...
			if (3 == ("" + a).charAt(0)) {
				if ($("#editBG:visible").length > 0) {
					var g;
					for (g = 0; g < I.length; g++) if (3 == I[g].type) {
						f = I[g];
						break
					}
					return f
			...
			return 1 == ("" + a).charAt(0) && (f = {
				...
				title: null,
				type: 2
			}), 4 == ("" + a).charAt(0) && (d = j(a, b), d.width = "100px", d.height = "100px", f = {
				content: "",
				css: d,
				id: e,
				num: 1,
				...
				title: null,
				type: "p"
			}), "v" == a && (d = j(a, b), d.width = "48px", d.height = "48px", f = {
				content: "",
				css: d,
				id: e,
				num: 1,
				...
				title: null,
				type: "v"
			}), c && $.extend(!0, f, c), I.push(f), J[f.id] = f, f
		}
		function n(a, b, d) {
			var e = G.wrapComp(b, "edit");
			$("#nr .edit_area").append(e);
			for (var f = G.getInterceptors(), g = 0; g < f.length; g++) f[g](e, b);
			G.getEventHandlers()[("" + a).charAt(0)](e, b), "g101" != d && (h.addPageHistory(H.obj.id, H.obj.elements), c.$broadcast("dom.changed"))
		}
		function o(a) {
			var b = [];
			return "g101" == a && (b.push(L("501")), b.push(L("502")), b.push(L("503")), b.push(L("601"))), b
		}
		function p(a, b) {
		...
		function t(a) {
			M || (M = d.open({
				...
				M = null, a.properties.title = b.title, a.properties.number = b.number;
				b.title.replace(/ /g, "&nbsp;");
				$.extend(!0, a.css, b.btnStyle), $("#" + a.id).length > 0 && $("#" + a.id).parents("li").remove(), n(a.type, a)
			}, function() {
				M = null, $("#" + a.id).length || (I.splice(I.indexOf(J[a.id]), 1), delete J[a.id])
			}))
		}
		function u(a) {
			M || (M = d.open({
				...
				M = null, b.type && (a.type = b.type), a.properties.placeholder = b.title, a.properties.required = b.required, a.title = b.title, $("#" + a.id).length > 0 ? ($("#" + a.id).attr("placeholder", b.title), $("#" + a.id).attr("required", b.required)) : n(a.type, a)
			}, function() {
				M = null, $("#" + a.id).length || (I.splice(I.indexOf(J[a.id]), 1), delete J[a.id])
			}))
		}
		function v(a) {
			M || (M = d.open({
				...
				M = null, a.properties = b;
				var c = $("#inside_" + a.id);
				c.length && c.remove(), n(a.type, a)
			}, function() {
				M = null, $("#" + a.id).length || (I.splice(I.indexOf(J[a.id]), 1), delete J[a.id])
			}))
		}
		function w(a) {
			M || (M = d.open({
				...
				M = null, a.properties.src = b, $("#" + a.id).length || n(a.type, a)
			}, function() {
				M = null, $("#" + a.id).length || (I.splice(I.indexOf(J[a.id]), 1), delete J[a.id])
			}))
		}
		function x(a) {
			d.open({
				...
				$("#" + a.id).length > 0 ? (a.properties.labels = [], b.forEach(c, function(b) {
				...
				}), $("#" + a.id).parents("li").remove(), n(a.type, a)) : (a.css = {
					left: "0px",
					width: "100%",
					bottom: "0px",
					height: "50px",
					...
					delete b.selected, delete b.mousedown, delete b.$$hashKey, a.properties.labels.push(b)
				}), position = null, n(a.type, a))
			}, function() {
				$("#" + a.id).length || (I.splice(I.indexOf(J[a.id]), 1), delete J[a.id]), console.log(a)
			})
		}
		function y(a) {
			A(a, function(b) {
				...
				"backgroundColor" == b.type && (c.style.backgroundImage = "none", c.style.backgroundColor = b.color, a.properties.imgSrc = null, a.properties.bgColor = b.color), h.addPageHistory(H.obj.id, H.obj.elements), $("#editBG").unbind("click"), $("#editBG").show().bind("click", function() {
					y(a)
				})
			}, function() {})
		}
		function z() {
			M || (M = d.open({
				...
				resolve: {
					obj: function() {
						return H.obj.scene.image && H.obj.scene.image.bgAudio ? H.obj.scene.image.bgAudio : {}
					}
				}
			}).result.then(function(a) {
				M = null, "bgAudio" == a.compType && (H.obj.scene.image || (H.obj.scene.image = {}), H.obj.scene.image.bgAudio = a.bgAudio)
			}, function() {
				M = null
			}))
		}
		function A(a, b, c) {
			if (!M) {
				var e = "0";
				3 == a.type && (e = "0"), 4 == a.type && (e = "1"), M = d.open({
					windowClass: "console img_console",
					templateUrl: "scene/console/bg.tpl.html",
					controller: "BgConsoleCtrl",
					resolve: {
		...
		function B(a) {
			F.currentElemDef = a, c.$broadcast("showStylePanel", {
				activeTab: "style"
			})
		}
		function C(a) {
		...
		function D(a) {
			console.log(a), F.currentElemDef = a, N = c.$broadcast("showCropPanel", a)
		}
		function E(a) {
			a.sceneId = H.obj.sceneId, d.open({
				windowClass: "console",
			...
			var F = {},
				G = eqShow.templateParser("jsonParser"),
				H = null,
				I = null,
				J = {};
			$("#nr .edit_area")
		}
		F.historyBack = function() {
			h.canBack(H.obj.id) && (I = h.back(H.obj.id), i(I))
		}, F.historyForward = function() {
			h.canForward(H.obj.id) && (I = h.forward(H.obj.id), i(I))
		}, $(document).keydown(function(a) {
			$("#nr .edit_area").length && ((a.ctrlKey || a.metaKey) && 90 == a.keyCode && F.historyBack(), (a.ctrlKey || a.metaKey) && 89 == a.keyCode && F.historyForward(), !a.ctrlKey && !a.metaKey || 86 != a.keyCode || !F.elemDefTpl || $("#btn-toolbar")[0] || $(".modal-dialog")[0] || (a.preventDefault(), q && (a.preventDefault(), F.pasteElement(F.originalElemDef, F.copyElemDef))), !a.ctrlKey && !a.metaKey || 67 != a.keyCode || !F.elemDefTpl || $("#btn-toolbar")[0] || $(".modal-dialog")[0] || (a.preventDefault(), F.pageId = H.obj.id, F.sameCopyCount = 0, F.copyElement(F.elemDefTpl)), c.$apply())
		});
		var K = F.createCompGroup = function(a, b) {
				for (var d = o(a), e = 0; e < d.length; e++) {
					var f = m(d[e].type, b, d[e]);
					b = null, n(d[e].type, f, "g101")
				}
				h.addPageHistory(H.obj.id, H.obj.elements), c.$broadcast("dom.changed")
			},
			L = function(a) {
				var b;
				return "501" == a && (b = {
		...
		F.createComp = function(a, b) {
			var c;
			if ("g" == ("" + a).charAt(0)) return void K(a, b);
			if ("9" == ("" + a).charAt(0)) return void z();
			if (1 == a) return $(".comp_title").length > 0 ? void alert("已存在一个标签") : (c = m(a, b), void x(c));
			if (c = m(a, b), 4 == a) return void r(c);
			if (5 == a) return void u(c);
			if (8 == a) return void t(c);
			if ("p" == a) return void v(c);
			if ("v" == a) return void w(c);
			if (3 == a) y(c);
			else {
				n(a, c)
			}
		}, F.updateCompPosition = function(a, b) {
			for (var d = 0; d < I.length; d++)"inside_" + I[d].id == a && (I[d].css ? (I[d].css.left = b.left, I[d].css.top = b.top, h.addPageHistory(H.obj.id, I)) : (I[d].css = b, h.addPageHistory(H.obj.id, I)));
			c.$apply()
		}, F.updateCompAngle = function(a, b) {
			for (var d = 0; d < I.length; d++)"inside_" + I[d].id == a && (I[d].css ? I[d].css.transform = "rotateZ(" + b + "deg)" : I[d].css = {}, h.addPageHistory(H.obj.id, I));
			c.$apply()
		}, F.getPageNames = function(b) {
		...
		}, F.updateCompSize = function(a, b) {
			for (var d = 0; d < I.length; d++)"inside_" + I[d].id == a && (I[d].css || (I[d].css = {}), I[d].css.width = b.width, I[d].css.height = b.height, I[d].properties.width = b.width, I[d].properties.height = b.height, b.imgStyle && (I[d].properties.imgStyle = b.imgStyle), h.addPageHistory(H.obj.id, I));
			c.$apply()
		}, F.savePageNames = function(b) {
			var c = "?c=page&a=savePageName",
				d = {
					id: b.id,
					sceneId: b.sceneId,
					name: b.name
			...
			$("#nr .edit_area li").each(function(a, b) {
				var c = J[b.id.replace(/inside_/g, "")];
				c && (c.css || (c.css = {}), c.css.zIndex = b.style.zIndex ? b.style.zIndex : "0")
			})
		}, F.copyElement = function(a) {
			var c = b.copy(a);
			q = !0, F.originalElemDef = a, F.copyElemDef = c
		}, F.pasteElement = function(a, c) {
			c.id = l(), c.pageId = H.obj.id, F.pageId == c.pageId ? k(a, c) : (F.sameCopyCount = 0, c.css = b.copy(a.css));
			var d = b.copy(c);
			I.push(d), J[d.id] = d, n(d.type, d), F.pageId = H.obj.id
		};
		var M = null,
			N = null;
		return G.addInterceptor(function(a, d, e) {
			function f() {
				...
				return q && b.find(".copy").after($('<li class="paste" role="presentation"><a role="menuitem" tabindex="-1"><div class="fa fa-paste" style="color: #08a1ef;"></div>&nbsp;&nbsp;粘贴</a></li>')), b.find(".edit").click(function(c) {
					switch (c.stopPropagation(), d.type.toString().charAt(0)) {
					case "1":
						break;
					case "2":
						p(a.find(".element").get(0), d);
						break;
					case "3":
						break;
					case "4":
					...
					c.stopPropagation(), B(d, function(b) {
						if (1 == d.type) for (var c in d.properties.labels) b.backgroundColor && (d.properties.labels[c].color.backgroundColor = b.backgroundColor, $(".label_content").css("background-color", b.backgroundColor)), b.color && (d.properties.labels[c].color.color = b.color, $(".label_content").css("color", b.color));
						else $(".element-box", a).css(b), $.extend(!0, d.css, b)
					}), b.hide()
				}), b.find(".animation").click(function(a) {
					a.stopPropagation(), C(d, function(a) {
						d.properties.anim = a
					}), b.hide()
				}), b.find(".link").click(function(a) {
					a.stopPropagation(), E(d), b.hide()
				}), b.find(".remove").click(function(e) {
					e.stopPropagation(), h.addPageHistory(H.obj.id, I), a.remove(), I.splice(I.indexOf(J[d.id]), 1), h.addPageHistory(H.obj.id, I), INTERVAL_OBJ[d.id] && (clearInterval(INTERVAL_OBJ[d.id]), delete INTERVAL_OBJ[d.id]), b.hide(), c.$apply(), c.$broadcast("hideStylePanel")
				}), b.find(".down").click(function() {
					var b = a.prev();
					if (!(b.length <= 0)) {
						var c = a.css("zIndex");
						a.css("zIndex", b.css("zIndex")), b.css("zIndex", c), b.before(a);
						for (var e = 0; e < I.length; e++) if (I[e].id == d.id && e > 0) {
							var f = I[e].css.zIndex;
							I[e].css.zIndex = I[e - 1].css.zIndex, I[e - 1].css.zIndex = f;
							break
						}
					}
					...
					if (!(b.length <= 0)) {
						...
						for (var e = 0; e < I.length; e++) if (I[e].id == d.id && e < I.length - 1) {
							var f = I[e].css.zIndex;
							I[e].css.zIndex = I[e + 1].css.zIndex, I[e + 1].css.zIndex = f;
							break
						}
					...
					a.stopPropagation(), F.sameCopyCount = 0, F.pageId = H.obj.id, $(".modal-dialog")[0] || F.copyElement(d), b.hide()
				}), b.find(".paste").click(function(a) {
					a.stopPropagation(), $(".modal-dialog")[0] || F.pasteElement(F.originalElemDef, F.copyElemDef), b.hide()
				}), b.find(".cut").click(function(a) {
					a.stopPropagation(), D(d), b.hide()
				}), 4 != d.type && (b.find(".link").hide(), b.find(".cut").hide()), "p" == d.type && (b.find(".animation").hide(), b.find(".style").hide()), b
			}
			if ("view" != e) {
				var g = $("#eq_main");
				a.on("click contextmenu", ".element-box", function(a) {
					a.stopPropagation(), $("#btn-toolbar")[0] || (F.elemDefTpl = b.copy(d)), $("#comp_setting:visible").length > 0 && "p" != d.type && (F.currentElemDef = d, c.$broadcast("showStylePanel"));
					var e = f(),
						h = $("#popMenu");
					return h.length > 0 && h.remove(), g.append(e), e.css({
						left: a.pageX + g.scrollLeft() + 15,
						top: a.pageY + g.scrollTop()
					}).show(), g.mousemove(function(a) {
						e = $("#popMenu"), (a.pageX < e.offset().left - 20 || a.pageX > e.offset().left + e.width() + 20 || a.pageY < e.offset().top - 20 || a.pageY > e.offset().top + e.height() + 20) && (e.hide(), $(this).unbind("mousemove"))
					}), !1
				}), a.attr("title", "按住鼠标进行拖动，点击鼠标进行编辑")
			}
		}), G.bindEditEvent("1", function(a, b) {
			$(a).unbind("dblclick"), $(a).show().bind("dblclick", function() {

				x(b)
			})
		}), G.bindEditEvent("2", function(a, b) {
			var c = $(".element", a)[0];
			$(c).mousedown(function(a) {
				...
				p(c, b), $("#popMenu").hide(), a.stopPropagation()
			})
		}), G.bindEditEvent("3", function(a, b) {
			$("#editBG").unbind("click"), $("#editBG").show().bind("click", function() {
				y(b)
			})
		}), G.bindEditEvent("v", function(a, b) {
			var c = $(".element", a)[0];
			$(c).unbind("dblclick"), $(c).bind("dblclick", function() {
				w(b), $("#popMenu").hide()
			})
		}), G.bindEditEvent("4", function(a, b) {
			var c = $(".element", a)[0];
			$(c).unbind("dblclick"), $(c).bind("dblclick", function() {
				r(b), $("#popMenu").hide()
			})
		}), G.bindEditEvent("5", function(a, b) {
			var c = $(".element", a)[0];
			$(c).unbind("dblclick"), $(c).bind("dblclick", function() {
				u(b), $("#popMenu").hide()
			})
		}), G.bindEditEvent("p", function(a, b) {
			var c = $(".element", a)[0];
			$(c).unbind("dblclick"), $(c).bind("dblclick", function() {
				v(b), $("#popMenu").hide()
			})
		}), G.bindEditEvent("6", function(a, b) {
			var c = $(".element", a)[0];
			$(c).unbind("dblclick"), $(c).bind("dblclick", function() {
				s(b), $("#popMenu").hide()
			})
		}), G.bindEditEvent("8", function(a, b) {
			var c = $(".element", a)[0];
			$(c).unbind("dblclick"), $(c).bind("dblclick", function() {
				t(b), $("#popMenu").hide()
			})
		}), G.bindEditEvent("7", function(a, b) {
			var c = $(".element", a)[0];
		...
		}), F.templateEditor = G, F.getTplById = function(b) {
		...
		}, F.saveSceneSettings = function(b) {
		...
		}, F.createBlankScene = function(b, c, d) {
			var e = {
				name: b,
				type: c,
				pageMode: d
		...
		}, F.getSceneByPage = function(b, c, d) {
			var f = "";
			...
			return f += (/\?/.test(f) ? "&" : "?") + "time=" + h.getTime(), a({
				...
				g.resolve(a), H = a.data, H.obj.elements || (H.obj.elements = []), I = H.obj.elements;
				for (var b = 0; I && b < I.length; b++) J[I[b].id] = I[b]
			}, function(a) {
				g.reject(a)
			}), g.promise
		}, F.previewSceneTpl = function(b) {
		...
		}, F.getSceneTpl = function(b) {
			var c = g.get("tplCache") ? g.get("tplCache") : g("tplCache"),
				d = e.defer();
			if (c.get(b)) {
				var f = $.extend(!0, {}, c.get(b));
				f.data.obj.scene && f.data.obj.scene.image && f.data.obj.scene.image.bgAudio && (H.obj.scene.image || (H.obj.scene.image = {}), H.obj.scene.image.bgAudio = f.data.obj.scene.image.bgAudio);
				for (var h = 0; h < f.data.obj.elements.length; h++) {
					var i = f.data.obj.elements[h];
					i.id = Math.ceil(100 * Math.random()), i.sceneId = H.obj.sceneId, i.pageId = H.obj.id
				}
				I = f.data.obj.elements;
				for (var j = 0; j < I.length; j++) J[I[j].id] = I[j];
				d.resolve(f)
			} else {
				...
				k += (/\?/.test(k) ? "&" : "?") + "time=" + l.getTime(), a({
					...
					c.put(a.data.obj.id, $.extend(!0, {}, a)), a.data.obj.scene && a.data.obj.scene.image && a.data.obj.scene.image.bgAudio && (H.obj.scene.image || (H.obj.scene.image = {}), H.obj.scene.image.bgAudio = a.data.obj.scene.image.bgAudio);
					for (var b = 0; b < a.data.obj.elements.length; b++) {
						var e = a.data.obj.elements[b];
						e.id = Math.ceil(100 * Math.random()), e.sceneId = H.obj.sceneId, e.pageId = H.obj.id
					}
					I = a.data.obj.elements;
					for (var f = 0; f < I.length; f++) J[I[f].id] = I[f];
					d.resolve(a)
				}, function(a) {
					d.reject(a)
				})
		...
		}, F.createCustomComp = L, F.openAudioModal = z, F.getElements = function() {
			return I
		}, F.getSceneObj = function() {
			return H
		}, F.getTpls = function(b, c, d, e) {
		...
		}, F.getPageTpls = function(b, c, d, e, f, g) {
		...
		}, F.getPageTplTypesTwo = function(b, c) {
		...
		}, F.saveMyTpl = function(b) {
			var c = '?c=user&a=saveMyTpl'  //"/m/scene/page/mytpl/save";
			return a({
				withCredentials: !0,
				method: "POST",
				url: JSON_URL + c,
		...
		}, F.previewScene = function(b) {
			var c = "?c=user&a=getMyTpl&id=" + b,  ///m/scene/page/mytpl/list/
				d = new Date;
		...
		return b.getDataBySceneId = function(b, c, d, e, f) {
			...
			return a({
				withCredentials: !0,
				method: "GET",
				url: PREFIX_URL + "promotion-code.htm?" + b + "&time=" + c.getTime()//m/u/promotion?code=
			})
		}, b
