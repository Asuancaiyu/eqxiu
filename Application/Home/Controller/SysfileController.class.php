...
class SysfileController extends Controller {
	...
	public function upload(){
			...
			foreach($jsonresult['list'] as $v){
				$data = file_get_contents('http://www.48ym.com/'.$v['path']);
				$datathumb = file_get_contents('http://www.48ym.com/'.$v['tmbPath']);
				$filetime = time();
	...
	public function createsyspage(){
			...
			$jsonresult = json_decode($jsonstr,true);
			foreach($jsonresult['list'] as $v){
				$data = file_get_contents('http://www.48ym.com/'.$v['properties']['thumbSrc']);
				$filetime = time();
	...
	public function createsysscene(){
			...
			$jsonresult = json_decode($jsonstr,true);
			foreach($jsonresult['list'] as $v){
				$data = file_get_contents('http://www.48ym.com/'.$v['image']['imgSrc']);
				$filetime = time();
				...
				{
					$data = file_get_contents('http://www.48ym.com/'.$v['image']['bgAudio']['url']);
					$filetime = time();
...
//	}
    ...
    public function createtplurl() {
		...
		foreach($m_scenelist as $v){
			echo "http://www.48ym.com/eqs/s/".$v['eqcode']."\n";
		}
	}
